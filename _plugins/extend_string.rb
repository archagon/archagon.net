...
#
# Version history:
#   * 1.0.3ab : September 2 2014
#               Removed some warnings. Added custom replacement character for spaces in urlize.
#   * 1.0.3 : July 23 2009
...
class String
  ...
  def removeaccents    
    ...
    String::ACCENTS_MAPPING.each {|letter,accents|
      ...
      rxp = Regexp.new("[#{packed}]", nil) # AB: removed 'U' option
      str.gsub!(rxp, letter)
    }
  ...
  def urlize(options = {})
    ...
    replacement_char = options[:convert_spaces].is_a?(String) ? options[:convert_spaces] : '_'
    str.gsub!(/\ /, replacement_char) if options[:convert_spaces] # AB: changed character
