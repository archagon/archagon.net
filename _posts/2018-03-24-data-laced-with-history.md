...
After musing over my bullet points, it occurred to me that the network problems I was dealing with—background cloud sync, editing across multiple devices, real-time collaboration, offline support, and reconciliation of distant or conflicting revisions—were all suggesting the same question: was it possible to design a system where any two revisions of the same document could be merged deterministically and sensibly without requiring user intervention? *Sync*, per se, wasn't the problem, since existing sync systems were ubiquitous, battle-hardened, and well-understood. What was concerning about most of them was that for almost any problem involving the network—merge conflicts, connectivity issues, user account switches—you'd end up in a complex conversation between the network, model, persistence, and UI layers to get everything back into a consistent state. The data couldn't be left alone to live its peaceful, functional life: the network necessarily made any dealings with it a stateful, urgent, and cross-architectural matter. On the other hand, if two documents could always be made to merge, then most of that coordination hullabaloo could go out the window. Each part of the system could work at its own pace.
...
CRDTs offered our documents the opportunity to manage their own sync and collaboration, wrestling control from centralized systems back to their rightful owners. The road here was fresh and unpaved, however, and I had to figure out if I could use these structures in a performant and space-efficient way for non-trivial applications.
...
When I first started reading up on CRDTs, it was unclear to me how conflict resolution was formalized. Every CRDT seemed to do something a bit different, and it was rare to find an approach that the developer could tweak depending on their needs. In CTs, the answer is refreshingly simple: conflicts occur when an atom has more children than expected. Combined with well-defined atoms, the intent for conflict resolution can be encoded at a higher level than would otherwise be possible.
...
(Also, observe how much of this is made possible by the integrated nature of the CT. If we were to store the transform for each point in a separate ORDT register, we would probably be limited to a simple sum or LWW for conflict resolution, since the register would know nothing about the purpose of its value or its relationship to the rest of the document.)
