...
* If using the object-based approach, ensure that each operation only exists in a single internal data structure at a time. Don't rely on "incidental state" such as insertion order; keep your internal data structures organized, sorted, and balanced at all times. Avoid moving operations between internal data structures. Instead of thinking of your object as having state, treat it as an organizational structure for your immutable operations: a kind of advanced frame.
...
With that said, the need to design a new ORDT should be relatively rare. Most documents can be designed through the composition of existing ORDTs, e.g. sequences and maps. Nonetheless, my feeling is that certain kinds of interactive software—drawing, music, and games—would be impossible to adapt to CRDTs without using this approach. And in any case, it's a fun topic to think about!
