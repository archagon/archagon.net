...
I didn't even think such a thing was possible, but CRDTs have proven to be that mythical, all-encompassing convergent data type I set out to look for. You can use them in practically any computing environment and they will happily merge. They work offline just as well as online. They're relatively easy to understand without having to write a dissertation. They're composable with each other. You can use them for real-time collaboration, cloud sync, or local file sharing — all without requiring any network coordination.
...
Sure, you have some tradeoffs compared to server-oriented sync techniques. For instance, CRDT data is always "live", even while offline. A user could accidentally edit their document on two offline devices, then find that they've merged into a mess on reconnection without any way to revert. For good and ill, users will never see the familiar version conflict dialog box. The lack of a guaranteed server also gives malicious users a whole lot more power, since they can irrevocably screw up a document without any possibility of a rollback. Servers can better manage resources by sending partial updates or by only giving a user the data they actually need. You'll also be hard-pressed to avoid servers where large amounts of data are concerned, such as in screen sharing or video editing.

But in exchange for a totally peer-to-peer computing future? A world full of systems finally able to freely collaborate with one another?
...
# Snippets & Ref Material

1. read through quickly
2. hide end stuff
3. rough draft publish
4. go through apocrypha list
5. read whole thing again
6. diagrams
...
TODO: why are we dealing with operations at all here? is it prudent to think of this as "reshuffling a straight UUID+timestamp sorted array while maintaining the same result"? maybe it's best to think of it as "operations in a special order", regardless of any hypothetical UUID+timestamp array??? i.e. data structure thinking vs. reorder-y thinking. how does the person acquire the wisdom and intuition to perform this restructuring step? also — producing output array vs. using it directly? it feels there are three equivalent things here: data structure from straight array <-> data structure from ordered array <-> data structure AS ordered array.



<direct use as low-level data structure? e.g. bitmap or cstring>

<gc: It is essential that the final state of the system is independent of the order of events!!>



wisdom in building data structures out of atomic, tagged operations: never any need to "merge" remote changes into working copy, since both are one and the same

hard part of sync handled in design stage: how do we atomize our data?
...
dichotomy between units of atomic data and operations that need to execute???

​	* both: clearly atoms are often represented as operations (insert, delete, update), but these are often equivalent to data atoms with location information



lexing/parsing vs. straight blitting — serialization
...
relationship between operations and data; one view = operations modify black box data structure; my view = operations directly form the data structure, and are queryable based on their organization



diagram of ordinary document sync vs. crdt sync w/o serialization



TODO: cvrdts vs cmrdts; emphasize that both can be used, and tradeoffs in each
...
are we forming an array/dictionary/etc — or are we creating an entirely new data structure? did i get sidetracked here? is this simply a new type of data structure algotether? why am i even thinking in terms of producing array/dictionary etc. — i think it's because there's this idea of some idyllic abstract data type that we're perfomring operations on



note: editing same doc on same device, multi device, etc. — opendoc



global order: a) easy to create, b) easy to parse



another way to think about it is each data structure carrying around with it all its sync metadata



data type centric design: operations encode what we know about a data type and commutify those unique operations; reconciliation does not require server; "CRDTs (convergent replicated datatypes) exploit knowledge regarding the commutativity and associativity of specific operations on specific datatypes" — use type-unique operations to ensure commutativity of changes — http://book.mixu.net/distsys/eventual.html



op-based crdts for sets? max? — ops collapse into intelligent conflict resolvers, but have the option of retaining their history 

(MAX 5) (MAX 2) (MAX 3) -> 5 — order doesn't matter for end result; order does matter for intermediary results; is not applied in arbitrary order in practice

* after (MAX 2) (MAX 3)
  * 3 + (MAX 5) — condensed to result
  * (MAX 3) + (MAX 5) — reduced operations
  * sorted[ (MAX 2) (MAX 3) ] + (MAX 5) — organized operations

(ADD 'A') (ADD 'C') (ADD 'B') -> { A C B } — efficient set construction

* after (ADD 'A') (ADD 'C')
  * { A C } + (ADD 'B') — condensed to result
  * tree[ (ADD 'A') (ADD 'C') ] + (ADD 'B') — organized operations
  * no need to condense baseline when data structure is comprised entirely of operations!

  ​

in a traditional system, concurrent operations would continuously mutate a single data structure and converge on the same result (CmRDT); with this system we've abstracted our data types into atomic operations and as such have a lot more flexibility: no need to condense operations, can condense without using a second resulting data type, can rebaseline more effectively

operations act on data -> operations ARE data by virtue of their structure and simple operations applied to those structures (e.g. 'get last' or 'read in order'); really a meta structure of sorts

back to operational thinking: executing operations in a linearized order vs. treating the operations as a data type? amazon multi-register w/all concurrent ops exposed?



fault: clock limit, not practical



database level thinking

* databases have fields
* remote concurrent changes need to be resolved
* how to do eventual consistency?
  * typical lww: use a vector clock
  * different fields have different operations other than get/set: strings have insert/replace, bitmaps have change pixel, counters have increment/decrement, etc.
  * intent is only conveyed if concurrent operations resolve sensibly, i.e. *merge*
  * let's build data structures out of operations!
    * how do we make it efficient to merge?
    * arrange operations to resemble the data structures themselves
  * sort of high level data structure notation
* requires smart leaves
* abstract data type thinking
* pure data
* spatial locality

datatype level thinking

* data types might need stability under concurrent changes, for many reasons (networking, multiprocess access, etc.)
* data types also need to remain highly computation-efficient and memory-efficient
* data types aren't just part of some distant data layer, but are directly used as building blocks for larger objects and data structures
* living state — classes and objects
* implementation thinking
* how to do eventual consistency?
  * expand existing data structures with metadata, while preserving structures in memory

in both cases we're dealing with data structures decomposable into atomic operations

stateful (coordinating) vs. stateless (crdt op-based) systems



i think the difficulty is this: we're using a structural approach that's used in two very different contexts and hierarchies, but for similar purposes

* need to store atomic metadata with data structures so that we can always merge
* general approach: decompose data structures into ops, then arrange those ops such that merge and data access is efficient
* then we split off depending on our use case: abstraction, flexibility, and extensibility (slow, streams of ops, gargantuan data layer, flexible rdt reducer layer) vs. performance and implementation (fast, low-level access, merges, and runtime use)
* former use case also allows us to e.g. cache operations, send around substreams of operations, create patches, etc. — whereas you can't really split those off from our data structure approach, since each operational slot is unique (i.e. each "operation" struct is actually e.g. array::operation or rope::operation or map::operation type, stateful, not functional — inversion of concepts)
* literal concept (operations put through functions and pruned/rearranged) vs. conceptual model (data structures comprised of operational structs)


good thing about operational approach: rebasing made easy, can rewrite history efficiently since history == data structure



write more about lamport timestamps

- vector clocks allow you to determine full causality information between any two operations, but require knowing every existing site (?) and take up a lot of space — untenable per letter
- lamport timestamps lose information, but combined with causal information, still allow you to induce a sensible, run-based total order
- vector clocks tell you about what each site has seen; direct causality just gives you that info directly, by definition
- cause = both data structure position AND vector clock surrogate? one of consequences of data structure being operational state??????
- vector clocks have a better linearized order, but does this really matter for convergent data structures? all we need is a differentiator for concurrent op tiebreaking, since we need causing ops anyway





consistent cut vs consitent in data model cut
...
One more thing to note. PORDT mentions that there are certain kinds of RDTs that don't require each operation to carry around a site UUID and timestamp. By virtue of their simplicity, they are commutative by default. One such example is an increment-only counter: as long as each increment operation is accounted for by the broadcast layer, the result will converge on all sites. There's no order to the operations — only a sum. The paper labels this category of RDTs **pure op-based CRDTs**. If you wish to use weft operations on these RDTs, keeping those UUIDs and timestamps around for each operation is still required!





* networked systems generally need coordination -- get rid of coordination, don't think of it in a distrbuted way
* crdts = generic conflict resolution for registers beyond just timestamps?
* cmrdt invariants -- easy, updates already batched, not sent out individually
