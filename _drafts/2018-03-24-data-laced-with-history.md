...
* If garbage collection is to be used, ensure that only operations causally prior and concurrent to any delete-type operation could possibly be affected by its compaction—and none afterwards. In other words, once a site has received a command that's intended to remove data, make sure that new commands can't be generated that reference the deleted portion of the ORDT.
