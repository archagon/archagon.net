...
task :new_post, :title do |t, args|
  resolved_posts_dir = "#{posts_dir}"
  if split_yaml_enabled
    resolved_posts_dir = "#{posts_raw_dir}"
  end
  ...
  filename = "#{resolved_posts_dir}/#{Time.now.strftime('%Y-%m-%d')}-#{title.to_url}.#{new_post_ext}"
  filename_metadata = "#{resolved_posts_dir}/#{Time.now.strftime('%Y-%m-%d')}-#{title.to_url}.#{new_post_metadata_ext}"
  if !split_yaml_enabled
  ...
  if split_yaml_enabled
    if File.exist?(filename_metadata)
      abort("rake aborted!") if ask("#{filename_metadata} already exists. Do you want to overwrite?", ['y', 'n']) == 'n'
    end
  end

  puts "Creating new post: #{filename}"
  if split_yaml_enabled
    puts "Creating new post metadata: #{filename_metadata}"
  end

  open(filename, 'w')
  open(filename_metadata, 'w') do |post|
    post.puts "---"
